...
class JeopardyGame {
    ...
    updateOptionsUI() {
        ...
        if (timerWhoami) timerWhoami.value = this.timerWhoamiSeconds;
        if (timerTruefalse) timerTruefalse.value = this.timerTruefalseSeconds;
    }
    ...
    updateManualPointsPanel() {
        ...
        this.players.forEach((p, i) => {
            if (!p.isHost) {
                ...
                opt.value = i;
                opt.textContent = (p.emoji ? p.emoji + ' ' : '') + p.name + ' (' + p.score + ' pts)';
    ...
    kickPlayer(playerIndex) {
        ...
        this.joinedNames.delete(player.name);
    ...
    loadGameState(data) {
        ...
        this.timerOptionsSeconds = data.timerOptionsSeconds || 15;
        this.timerAnagramSeconds = data.timerAnagramSeconds || 20;
        this.timerWhoamiSeconds = data.timerWhoamiSeconds || 20;
        this.timerTruefalseSeconds = data.timerTruefalseSeconds || 10;
    }
    ...
    // [ELIMINAR] Estas funciones ya no se usan pero las dejamos para no romper nada
    createBoard() {
        // Función obsoleta - mantener vacía
    }
    ...
    showToast(msg) {
        const ex = document.querySelector('.export-toast');
        if (ex) ex.remove();
    ...
    adjustManualPoints(multiplier) {
        ...
        const amount = parseInt(input.value) * multiplier;
    ...
    showResults() {
        ...
        podium.innerHTML = '';
        if (sorted.length === 0) {
            podium.innerHTML = '<p class="hint">No hay jugadores</p>';
        } else {
            ...
            order.forEach(({ p, pos }) => {
                ...
                card.innerHTML = `<div class="pos">${emoji} #${pos}</div><div class="name">${p.emoji ? p.emoji + ' ' : ''}${p.name}</div><div class="pts">${p.score} pts</div>`;
    ...
    toggleFullResults() {
        ...
        if (el.classList.contains('hidden')) {
            ...
            if (this.sortedPlayers.length === 0) {
                el.innerHTML += '<p class="hint">No hay jugadores</p>';
            } else {
                this.sortedPlayers.forEach((p, i) => {
                    const row = document.createElement('div');
                    row.className = 'result-row';
                    row.innerHTML = `<span class="rank">#${i + 1}</span><span class="name-col">${p.emoji ? p.emoji + ' ' : ''}${p.name}</span><span class="pts-col">${p.score} pts</span>`;
                    el.appendChild(row);
                });
            }
    ...
    startTimer(questionType) {
        ...
        let seconds = this.timerTextSeconds;
        if (questionType === 'options') seconds = this.timerOptionsSeconds;
        else if (questionType === 'anagram') seconds = this.timerAnagramSeconds;
        else if (questionType === 'whoami') seconds = this.timerWhoamiSeconds;
        else if (questionType === 'truefalse') seconds = this.timerTruefalseSeconds;
    ...
    handleTrueFalseAnswer(isCorrect, selected) {
        ...
        const player = this.players[this.currentPlayer];
        this.showPointsAnimation(player.name, player.emoji || '', puntosFinales, isCorrect);
